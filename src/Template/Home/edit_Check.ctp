<?php
	...
	$sql = "UPDATE jushoroku SET name='{$this->request->getData('name_ec')}', furigana='{$this->request->getData('hira_ec')}', post='{$this->request->getData('post_ec')}', address='{$this->request->getData('address_ec')}' WHERE ID = {$this->request->getData('editid_e')} ";
	...
	if($flug){
		...
		echo $this->Form->control('messagetext', ['type' => 'hidden', 'value' => "編集完了"]);
		echo $this->Form->control('autobutton', ['type' => 'submit'] );
		echo $this->Form->end();
	}
	else {
		echo $this->Form->create('2', ['type' => 'post', 'url' => '/Home/edit']);
		echo $this->Form->control('editid', ['type' => 'hidden', 'value' => $id_up]);
